...
export default function Index({ products, auth }) { // คอมโพเนนต์ Index รับ props ที่ชื่อว่า products และ auth
    if (!auth.user) { // ตรวจสอบว่าไม่มีผู้ใช้ล็อกอิน
        // สามารถเพิ่มการเปลี่ยนเส้นทางไปยังหน้า login ที่นี่ได้
        return (
            <div>
                <p>Please log in to view the products.= http://127.0.0.1:8000/login </p>
            </div>
        );
    }

    return (
        <AuthenticatedLayout auth={auth}> {/* ส่ง auth ไปที่ AuthenticatedLayout */}
            <Head title="Products" /> {/* กำหนดชื่อหัวข้อของหน้าเป็น "Products" */}
